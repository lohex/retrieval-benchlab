...
def _latest_datasets(registry_path: Path) -> pd.DataFrame:
    ...
    SELECT
        ...
        datasets.n_queries,
        datasets.n_documents,
        datasets.n_relevance_relations,
    ...
    """
    datasets = _read_query(registry_path, query)
    if datasets.empty:
...
def _enrich_dataset_statistics(datasets: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in datasets.to_dict(orient="records"):
        ...
        n_positive_documents = _metadata_value(
            metadata,
            "n_positive_documents",
        )
...
def _registered_pipelines(registry_path: Path) -> pd.DataFrame:
    ...
    """
    pipelines = _read_query(registry_path, query)
    if pipelines.empty:
        raise ReportingError("The dataset registry contains no pipelines")

    model_short_names = pipelines["model_name"].str.rsplit(
        "/",
        n=1,
    ).str[-1]
...
def _result_tables(
    ...
    FROM metrics
    """
    runs = _read_query(results_path, runs_query)
    metrics = _read_query(results_path, metrics_query)
    return runs, metrics
...
def _pipeline_coverage(
    ...
    result["coverage"] = (
        result["evaluated_latest_datasets"]
        / result["available_latest_datasets"]
    )
    return result
...
def _latest_metric_values(
    ...
    metric_values = metric_values.rename(
        columns={
            "dataset_name_y": "dataset",
            "version": "version",
            "metric_name": "metric",
        }
    )
    return metric_values[columns].sort_values(
        ["metric", "pipeline_label", "dataset"]
    )
...
def load_registry_report(
    ...
    if not registry_path.is_file():
        raise ReportingError(
            f"Registry database does not exist: {registry_path}"
        )
    if not results_path.is_file():
        raise ReportingError(
            f"Results database does not exist: {results_path}"
        )
    if registry_path == results_path:
        raise ReportingError(
            "registry_db_path and results_db_path must differ"
        )
    ...
    pipeline_statistics = _pipeline_coverage(
        registered_pipelines,
        runs,
        latest_dataset_ids,
    )
    metric_values = _latest_metric_values(
        latest_datasets,
        registered_pipelines,
        runs,
        metrics,
